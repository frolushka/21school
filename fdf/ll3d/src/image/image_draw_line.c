...
/*   Updated: 2019/02/11 23:22:17 by sbednar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
				...
				t_vec2 const *v2, t_vec2 const *c)
{
   ...
   /* Special-case horizontal, vertical, and diagonal lines, which
      require no t[4] because they go right through the center of
      every pixel */
   if ((t[7] = t[13] - t[12]) == 0) {
      /* Horizontal line */
      while (t[6]-- != 0) {
   ...
   t[2] = 0;  /* initialize the line error accumulator to 0 */
   /* # of bits by which to shift t[2] to get intensity level */
   t[0] = 8;
   /* Mask used to flip all bits in an intensity t[4], producing the
      result (1 - intensity t[4]) */
   t[5] = 255;
   ...
   if (t[7] > t[6]) {
      ...
      t[1] = ((unsigned long) t[6] << 16) / (unsigned long) t[7];
      /* Draw all pixels other than the first and last */
      while (--t[7]) {
         t[3] = t[2];   /* remember current accumulated error */
         t[2] += t[1];      /* calculate error for next pixel */
         if (t[2] <= t[3]) {
            /* The error accumulator turned over, so advance the X coord */
            t[10] += t[9];
         }
         t[12]++; /* Y-major, so always advance Y */
         /* The IntensityBits most significant bits of t[2] give us the
            intensity t[4] for this pixel, and the complement of the
            t[4] for the paired pixel */
         t[4] = t[2] >> t[0];
         image_set_pixel(mlx->image, t[10], t[12], color_set_brightness(color_lerp(t[14], t[15], (float)ft_fabs(t[10] - t[16]) / ft_fabs(t[11] - t[16])), (255.0f - t[4]) / 255.0f));
		 image_set_pixel(mlx->image, t[10] + t[9], t[12], color_set_brightness(color_lerp(t[14], t[15], (float)ft_fabs(t[10] - t[16]) / ft_fabs(t[11] - t[16])), (255.0f - (t[4] ^ t[5])) / 255.0f)
		 );
      }
   ...
   t[1] = ((unsigned long) t[7] << 16) / (unsigned long) t[6];
   /* Draw all pixels other than the first and last */
   while (--t[6]) {
      t[3] = t[2];   /* remember current accumulated error */
      t[2] += t[1];      /* calculate error for next pixel */
      ...
      t[4] = t[2] >> t[0];
	  image_set_pixel(mlx->image, t[10], t[12], color_set_brightness(color_lerp(t[14], t[15], (float)ft_fabs(t[10] - t[16]) / ft_fabs(t[11] - t[16])), (255.0f - t[4]) / 255.0f));
	  image_set_pixel(mlx->image, t[10], t[12] + 1, color_set_brightness(color_lerp(t[14], t[15], (float)ft_fabs(t[10] - t[16]) / ft_fabs(t[11] - t[16])), (255.0f - (t[4] ^ t[5])) / 255.0f));
   }
