...
static void put_camera_info(const t_mlx *mlx, t_cam const *cam)
{
	...
	sprintf(a, "%-8.3f %-8.3f %-8.3f %-8.3f", cam->cam->val[0][0], cam->cam->val[0][1], cam->cam->val[0][2], cam->cam->val[0][3]);
	mlx_string_put(mlx->mlx, mlx->win, 1500, 70, 0xFFFFFF, a);
	sprintf(a, "%-8.3f %-8.3f %-8.3f %-8.3f", cam->cam->val[1][0], cam->cam->val[1][1], cam->cam->val[1][2], cam->cam->val[1][3]);
	mlx_string_put(mlx->mlx, mlx->win, 1500, 90, 0xFFFFFF, a);
	sprintf(a, "%-8.3f %-8.3f %-8.3f %-8.3f", cam->cam->val[2][0], cam->cam->val[2][1], cam->cam->val[2][2], cam->cam->val[2][3]);
	mlx_string_put(mlx->mlx, mlx->win, 1500, 110, 0xFFFFFF, a);
	sprintf(a, "%-8.3f %-8.3f %-8.3f %-8.3f", cam->cam->val[3][0], cam->cam->val[3][1], cam->cam->val[3][2], cam->cam->val[3][3]);
	...
	sprintf(a, "FOCUS:   %-7.3f   (keys 6+ 3-)%s", cam->focus, cam->mode == CAM_ISO ? " DISABLED" : "");
...
static void put_dots_info(const t_mlx *mlx, t_dots const *dots)
{
	...
	sprintf(a, "%-8.3f %-8.3f %-8.3f %-8.3f", dots->wb[dots->i]->x, dots->wb[dots->i]->y, dots->wb[dots->i]->z, dots->wb[dots->i]->w);
	mlx_string_put(mlx->mlx, mlx->win, 1500, 290, 0xFFFFFF, a);
	sprintf(a, "%-8.3f %-8.3f %-8.3f", dots->cb[dots->i]->x, dots->cb[dots->i]->y, dots->cb[dots->i]->z);
...
static void	check(const t_cam *cam)
{
	cam = cam + 1;
	return ;
	// printf("%s\n", (cam->piv->x - cam->cam->val[3][0] + cam->cam->val[]));
}
